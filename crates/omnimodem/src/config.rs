...
//! devices. Today it registers `rtl_tcp` SDR endpoints so `ListDevices` can
//! surface them for selection even though they are network endpoints that no
//! hardware enumeration produces. Ad-hoc binding of an unregistered
//! `rtltcp:host:port` still works without any config entry.
//!
...
/// Parse one non-comment directive. Currently only `rtl_tcp <host>:<port>
/// [label...]`.
fn parse_line(line: &str) -> Option<DeviceDescriptor> {
    let mut parts = line.split_whitespace();
    if parts.next()? != "rtl_tcp" {
        return None;
    }
    ...
    let label = if label.is_empty() { format!("rtl_tcp {host}:{port}") } else { label };
    Some(DeviceDescriptor { id, label, has_capture: true, has_playback: false })
}
