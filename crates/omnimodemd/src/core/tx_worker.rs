...
fn run(mut cfg: TxWorkerCfg, rx: Receiver<TxJob>, cancel: Arc<AtomicBool>) {
    while let Ok(job) = rx.recv() {
        ...
        let samples = match cfg.modulator.modulate(&job.frame) {
            Ok(s) => s,
            Err(e) => {
                // Payload not valid for this mode: emit start + a failure with the
                // reason (so the operator learns WHY there was no audio) + complete
                // (so a client awaiting this transmit id isn't left hanging).
...
mod tests {
    ...
    fn every_windowed_mode_plays_nonsilent_audio() {
        ...
        for (mode, msg) in cases {
            ...
            worker
                .enqueue(TxJob { frame: DspFrame::text(*msg), transmit_id: TransmitId(1) })
                .unwrap();
            let mut peak = 0i32;
            for _ in 0..600 {
                {
    ...
    fn unencodable_message_reports_transmit_failed() {
        ...
        worker
            .enqueue(TxJob { frame: DspFrame::text("CQ K7XYZ/P EM10"), transmit_id: TransmitId(7) })
            .unwrap();
