//! LDPC(174,87) tables for JS8, transcribed **verbatim** from the js8call
//! reference (upstream `js8call/js8call` @ a7ff1be). Generated by
//! `scratch/refvectors/js8/gen_js8_tables.py` — do not edit by hand.
//!
//! ref: lib/ft8/ldpc_174_87_params.f90 (`g`, `colorder`),
//!      lib/ft8/bpdecode174.f90 (`Nm`, `Mn`, `nrw`).
//!
//! The (174,87) code is a regular column-weight-3 PEG LDPC. `encode174`
//! forms `[parity(87) | message(87)]` then permutes columns by `COLORDER`;
//! the parity-check Tanner graph (`NM`/`NRW`/`MN`) operates on that permuted
//! codeword. All indices are kept in the reference's **1-origin** form and
//! rebased to 0 at the point of use.

/// Parity generator matrix rows: 87 rows of 22 hex chars (88 bits, first 87
/// used). Row `i` gives the message-bit contributions to parity bit `i`.
/// ref: ldpc_174_87_params.f90 `character*22 g(87)`.
pub const JS8_174_87_GEN: [&str; 87] = [
...
/// Column reorder applied by `encode174` (`codeword(colorder+1)=itmp`).
/// 0-origin codeword positions. ref: ldpc_174_87_params.f90 `colorder(174)`.
pub const JS8_174_87_COLORDER: [usize; 174] = [
    0, 1, 2, 3, 30, 4, 5, 6, 7, 8, 9, 10,
    11, 32, 12, 40, 13, 14, 15, 16, 17, 18, 37, 45,
    29, 19, 20, 21, 41, 22, 42, 31, 33, 34, 44, 35,
    47, 51, 50, 43, 36, 52, 63, 46, 25, 55, 27, 24,
    23, 53, 39, 49, 59, 38, 48, 61, 60, 57, 28, 62,
    56, 58, 65, 66, 26, 70, 64, 69, 68, 67, 74, 71,
    54, 76, 72, 75, 78, 77, 80, 79, 73, 83, 84, 81,
    82, 85, 86, 87, 88, 89, 90, 91, 92, 93, 94, 95,
...
/// Tanner graph, checks → codeword bits: `NM[c]` lists the (1-origin)
/// codeword indices in check `c`, zero-padded to 7; `NRW[c]` is the valid
/// count (5, 6, or 7). ref: bpdecode174.f90 `Nm(7,87)` / `nrw(87)`.
pub const JS8_174_87_NM: [[u8; 7]; 87] = [
...
/// Valid entries per check for [`JS8_174_87_NM`]. ref: bpdecode174.f90 `nrw`.
pub const JS8_174_87_NRW: [u8; 87] = [
    6, 6, 6, 6, 6, 6, 6, 6, 6, 6,
    6, 6, 6, 6, 6, 6, 6, 6, 6, 6,
    6, 6, 6, 6, 6, 6, 6, 6, 6, 6,
    6, 6, 6, 6, 6, 6, 6, 6, 6, 7,
...
/// Tanner graph, bits → checks: `MN[v]` lists the 3 (1-origin) checks that
/// include codeword bit `v` (column weight 3). ref: bpdecode174.f90 `Mn(3,174)`.
pub const JS8_174_87_MN: [[u16; 3]; 174] = [
    [1, 25, 69],
    [2, 5, 73],
    [3, 32, 68],
    [4, 51, 61],
