//! FT8/FT4 (174,91) LDPC constants, transcribed byte-for-byte from
//! `ft8_lib` (`kgoba/ft8_lib`, `ft8/constants.c`) — themselves the WSJT-X
//! tables. Generated mechanically from the upstream C source; do not hand-edit.
//!
//! - `FT8_LDPC_GENERATOR[c]`: parity-check `c`'s 91-bit generator row, bit-packed
//!   MSB-first into 12 bytes (top 91 bits used). Parity bit `c` of a systematic
//!   codeword is the XOR of the message bits selected by this row.
//! - `FT8_LDPC_NM[c]`: the 1-origin codeword indices in parity check `c`
//!   (zero-padded to 7); `FT8_LDPC_NUM_ROWS[c]` gives the valid count.

/// Parity generator matrix, 83 rows × 12 bytes (bit-packed, MSB first).
pub(super) const FT8_LDPC_GENERATOR: [[u8; 12]; 83] = [
...
/// Parity-check variable lists (1-origin codeword indices, zero-padded to 7).
pub(super) const FT8_LDPC_NM: [[u8; 7]; 83] = [
...
/// Number of valid entries in each row of [`FT8_LDPC_NM`].
pub(super) const FT8_LDPC_NUM_ROWS: [u8; 83] = [7, 6, 6, 6, 7, 6, 7, 6, 6, 7, 6, 6, 7, 7, 6, 6, 6, 7, 6, 7, 6, 7, 6, 6, 6, 7, 6, 6, 6, 7, 6, 6, 6, 6, 7, 6, 6, 6, 7, 7, 6, 6, 6, 6, 7, 7, 6, 6, 6, 6, 7, 6, 6, 6, 7, 6, 6, 6, 6, 7, 6, 6, 6, 7, 6, 6, 6, 7, 7, 6, 6, 7, 6, 6, 6, 6, 6, 6, 6, 7, 6, 6, 6];
