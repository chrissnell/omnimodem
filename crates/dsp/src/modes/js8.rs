...
mod tests {
    ...
    /// Cross-decode: our `Js8Demod` decodes the **reference JS8 waveform** for
    /// both Costas variants. The fixtures are generated by the unmodified
    /// js8call encoder (`encode174.f90` + the `genjs8` tone assembly + the
    /// `genjs8refsig` continuous-phase model) from the 87 msgbits of a "CQ
    /// K1ABC" data frame — see `scratch/refvectors/js8/refwave/`. This proves
    /// our Costas sync (original + symmetrical), tone demap, and LDPC(174,87)
    /// interoperate with the reference on the air. i16 @ 12 kHz, f0 = 1500 Hz.
    fn decode_ref_i16(bytes: &[u8], submode: Js8Submode) -> Vec<String> {
        let samples: Vec<f32> = bytes
            .chunks_exact(2)
            .map(|c| i16::from_le_bytes([c[0], c[1]]) as f32 / 32768.0)
            .collect();
        let win_len = (JS8_RATE as f32 * submode.params().tx_seconds as f32) as usize;
        let mut win = vec![0.0f32; win_len.max(samples.len())];
        win[..samples.len()].copy_from_slice(&samples);
        Js8Demod::new(submode)
            .decode_window(&win, 0)
            .iter()
            .filter_map(|f| match &f.payload {
    ...
    #[test]
    fn cross_decode_reference_normal() {
        let bytes = include_bytes!("../../tests/vectors/js8_ref_normal.i16");
        let texts = decode_ref_i16(bytes, Js8Submode::Normal);
        assert!(texts.iter().any(|t| t == "CQ K1ABC"), "ref Normal cross-decode failed; got {texts:?}");
    }

    #[test]
    fn cross_decode_reference_turbo() {
        let bytes = include_bytes!("../../tests/vectors/js8_ref_turbo.i16");
        let texts = decode_ref_i16(bytes, Js8Submode::Turbo);
        assert!(texts.iter().any(|t| t == "CQ K1ABC"), "ref Turbo cross-decode failed; got {texts:?}");
    }
