...
>
> **RX also proven (round-trip cross-decode).** `build_sstv_rx.sh` + `sstv_rx_dump.cxx` feed that TX audio into the **unmodified `CSSTVDEM`** started deliberately in the *wrong* mode (smR36): the reference demod detects the VIS header and switches to smSCT1, entering picture-RX (`SyncMode` 0→1→2→3→256) at sample 9978, and tracks the per-line 1200 Hz sync at the expected ~4722-sample cadence (`sstv_scottie1_rx.json`, `vis_ok: true`). This is the "reference decodes our TX" substitute (no Linux CLI decoder needed). **The F0 critical-path risk is fully retired — both directions run on the unmodified core.** **Remaining F0 work:** transcribe `DrawSSTVNormal` (Main.cpp:3715) so the RX driver also dumps the reconstructed pixel raster (bit-exact gate for T5), then extend the TX/RX drivers across all families at each T1.
