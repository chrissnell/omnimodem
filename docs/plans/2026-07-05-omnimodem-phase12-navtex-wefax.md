...
- `crates/dsp/src/modes/navtex.rs` — 100-baud 2-FSK (±85 Hz) front end (reuses `frontend::nco::DownConverter` + `frontend::fir::Fir` + `frontend::detector::FmDiscriminator`, as RTTY does), a synchronous 7-bit bit-clock, then `ccir476` FEC-B → `FramePayload::Text`. SITOR-B = raw stream; NAVTEX = adds ZCZC…NNNN message framing.
...
**Port shape.** `NavtexVariant { Navtex, SitorB }`. Native rate **8000 Hz** (100 baud → 80 samples/bit, integer). Center default 1000 Hz. TX: `ccir476::encode` + `create_fec` → LSB-first bits → `Fsk2`. RX: `DownConverter`→`Fir`→`FmDiscriminator`, a mid-bit sampler at 80 sps producing a soft mark/space per bit, `ccir476::FecBDecoder` fed bit-by-bit → text; NAVTEX strips the ZCZC/NNNN frame.
